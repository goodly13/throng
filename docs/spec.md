...
- **FR-045** Packages. `packaging/package.sh` builds packages for the platform it runs on:
  ...
  - **macOS:** a universal `throng.app` (Apple silicon and Intel) in a `.dmg`, and the same app in
    ...
    is what the in-app updater installs (FR-054). The app is signed with a Developer ID when those
...
- **FR-048** Publishing. A `v<version>` tag, where `<version>` is the workspace version in
  ...
  over every package, which the AppImage updater checks its download against (FR-054).
- **FR-054** Updates. While `updates.check` is on (the default) throng asks GitHub for the latest
