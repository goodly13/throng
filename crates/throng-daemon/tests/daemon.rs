//! The daemon against real shells on real PTYs.
#![cfg(unix)]
...
fn env() -> Vec<(String, String)> {
    let mut env: Vec<(String, String)> =
        std::env::vars().filter(|(k, _)| k == "PATH" || k == "HOME" || k == "LANG").collect();
...
impl View {
    ...
    fn count(&self, needle: &str) -> usize {
        String::from_utf8_lossy(&self.text).matches(needle).count()
    }
}
...
fn spawn_streams_output_and_reports_the_exit_code() {
    ...
    let client = h.client();
    let spec =
        spec(&h.cwd(), "/bin/sh", &["-c", "echo hello from $0; echo secret=${THRONG_SECRET:-none}; exit 3"]);
    let mut view = View::new(&snapshot(client.request(Request::Spawn(spec), WAIT)));
    view.pump(&client, |v| v.exited.is_some());
    assert!(view.contains("hello from /bin/sh"));
...
fn spawning_into_a_missing_folder_is_refused_in_words() {
    let h = Harness::start();
    let client = h.client();
    let spec = spec(&h.cwd().join("gone"), "/bin/sh", &[]);
...
fn an_exit_while_nobody_watches_is_kept_for_the_next_attach() {
    let h = Harness::start();
    let client = h.client();
    let spec = spec(&h.cwd(), "/bin/sh", &["-c", "sleep 0.3; echo last words; exit 7"]);
    ...
    assert!(String::from_utf8_lossy(&snap.tail).contains("last words"));
...
fn reattaching_never_duplicates_or_loses_output() {
    let h = Harness::start();
    let client = h.client();
    let script = "i=0; while [ $i -lt 400 ]; do echo line$i; i=$((i+1)); sleep 0.002; done; sleep 0.3";
    let spec = spec(&h.cwd(), "/bin/sh", &["-c", script]);
    let terminal = spec.terminal;
    ...
    view.pump(&client, |v| v.exited.is_some());
    let text = String::from_utf8_lossy(&view.text).replace('\r', "");
...
fn bash() -> Option<&'static str> {
    ["/bin/bash", "/usr/bin/bash"].into_iter().find(|p| Path::new(p).exists())
}
...
fn busy_means_a_command_holds_the_foreground_and_close_idle_spares_it() {
    ...
    let h = Harness::start();
    let client = h.client();
    let busy_spec = spec(&h.cwd(), bash, &["--norc", "--noprofile", "-i"]);
    let idle_spec = spec(&h.cwd(), bash, &["--norc", "--noprofile", "-i"]);
    ...
    pump_all(&client, &mut [&mut busy_view, &mut idle_view], |v| v.contains("$ "));
    assert!(!busy(&client, busy_id));

    client.write(busy_id, b"sleep 30\r");
...
fn startup_command_runs_once_after_first_output_and_never_on_reattach() {
    let h = Harness::start();
    let client = h.client();
    let mut spec = spec(&h.cwd(), "/bin/sh", &[]);
    spec.startup_command = Some("echo started-$((20+22))".into());
    let terminal = spec.terminal;
    let mut view = View::new(&snapshot(client.request(Request::Spawn(spec), WAIT)));
...
fn keystrokes_arrive_in_the_order_typed() {
    let h = Harness::start();
    let client = h.client();
    let spec = spec(&h.cwd(), "/bin/cat", &[]);
    let terminal = spec.terminal;
    let mut view = View::new(&snapshot(client.request(Request::Spawn(spec), WAIT)));
    let typed: String = (0..400).map(|i| char::from(b'a' + (i % 26) as u8)).collect();
    for byte in typed.bytes() {
        client.write(terminal, &[byte]);
    }
    client.write(terminal, b"\n");
...
fn the_pty_takes_the_smallest_attached_view() {
    ...
    a.write(terminal, b"stty size\n");
    view.pump(&a, |v| v.contains("20 70"));
    b.request(Request::Detach { terminal }, WAIT).unwrap();
    a.write(terminal, b"stty size\n");
    view.pump(&a, |v| v.contains("30 100"));
    // A resize from a view that is no longer attached must not pin the size.
    b.post(Request::Resize { terminal, cols: 10, rows: 5 });
    a.write(terminal, b"echo marker; stty size\n");
    view.pump(&a, |v| v.count("30 100") >= 2);
    assert!(!view.contains("5 10"));
...
fn shutdown_without_kill_all_refuses_while_terminals_run() {
    let h = Harness::start();
    let client = h.client();
    let spec = spec(&h.cwd(), "/bin/sh", &["-c", "sleep 30"]);
    let terminal = spec.terminal;
    snapshot(client.request(Request::Spawn(spec), WAIT));
    assert!(matches!(
